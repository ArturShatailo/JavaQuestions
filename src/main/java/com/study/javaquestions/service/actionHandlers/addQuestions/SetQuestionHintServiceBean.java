...
@Service
@AllArgsConstructor
public class SetQuestionHintServiceBean implements ActionHandlerService, BotSession, QuestionMenuSession, KeyboardButtons<String> {
    ...
    public boolean mineCheck(Request request) {
        String requestSessionStep = "QUESTION HINT";
    ...
    public void sendRequest(Request request) {
        ...
        sessionSteps.put(chatID, "QUESTION ANSWER");
        String hint = request.getSendMessage().getText();

        processQuestionSession(chatID, hint);

        showKeyboardButtons(request,
                "Дякую, тепер напиши текст відповіді і відправ мені",
                List.of("🔙 Повернутись до вибору теми"));
    }

    private void processQuestionSession(String chatID, String hint) {
        questionsSessions.get(chatID).setHint(hint);
        questionSessionServiceBean.updateHintByChatId(chatID, hint);
    }
